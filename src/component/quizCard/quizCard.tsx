...
export function QuizCard({ quizName, questionNumber }: quizCardProp) {
  ...
  useEffect(() => {
    ...
    if(questionNumber < 5)
      quizDispatch({type: "SET-TIMER", payload: 10})
  },[questionNumber])

  useEffect(() => {
    let timeout: NodeJS.Timeout
    if(quizState.timer > 0) {
      timeout = setTimeout(() => quizDispatch({type: "SET-TIMER", payload: quizState.timer - 1}), 1000)
    }
    else {
          ...
          <div key={option.value} className="options">
            <button 
              ...
              onClick={() => {
                ...
                quizDispatch({type: "CURRENT-QUIZ-SCORE", payload: {question: currentquestion, option: option}})
                }}>
                  {option.value}
            </button>
          </div>
